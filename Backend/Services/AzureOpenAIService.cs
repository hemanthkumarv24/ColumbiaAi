...
public interface IAzureOpenAIService
{
    ...
    Task<string> GetChatNameAsync(string messages, string? userContext = null);
}
...
public class AzureOpenAIService : IAzureOpenAIService
{
    ...
    public async Task<string> GetChatNameAsync(string messages, string? userContext = null)
    {
        var chatClient = _client.GetChatClient(_deploymentName);
        var chatMessages = new List<OpenAI.Chat.ChatMessage>();
        ...
        chatMessages.Add(new SystemChatMessage("Generate a short and relevant title (max 6 words) summarizing the following user message. No punctuation, no quotes.\";"));
        chatMessages.Add(new UserChatMessage(messages));
        //foreach (var msg in messages)
        //{
        //    if (msg.Role == "user")
        //    {
